...
@[simp]
def equiv_qf (φ : formula L) := ∃ ψ : qf L, (A∣Γ ⊢ φ) ↔ (A∣Γ ⊢ (ψ : formula L))

def Eq_equiv_qf {Γ : list (formula L)} : ((A∣Γ ⊢ p) ↔ (A∣Γ ⊢ q)) → (equiv_qf A Γ p → equiv_qf A Γ q) := begin
   intros h₁ h₂,
   rcases h₂ with ⟨φ₃, h₃⟩,
   existsi φ₃,
   split,
...
@[simp]
def qe := ∀ (φ : formula L), equiv_qf A Γ φ

/- If a theory Γ has quantifier elimination on conjunctions of literals with
   with a single existential quantifier -/
def qe_ecl1 := ∀ (φ : ecl1 L), equiv_qf A Γ (φ : formula L)

/- If a theory Γ has quantifier elimination on disjunctions of conjunctions
   of literals with a single existential quantifier -/
def qe_edcl1 := ∀ (φ : edcl1 L), equiv_qf A Γ (φ : formula L)

/- If a theory Γ has quantifier elimination on disjunctions of conjugations 
   of literals with a single quantifier -/
@[simp]
def qe_qdcl1 := ∀ (φ : qdcl1 L), equiv_qf A Γ (φ : formula L)

@[simp]
def qe_dnf := ∀ (φ : dnf L), equiv_qf A Γ (φ : formula L)

/- If a theory Γ has quantifier elimination on quantifier free formulas -/
@[simp]
def qe_qf := ∀ (φ : qf L), equiv_qf A Γ (φ : formula L)

/- All theories have quantifier elimination on quantifier free formulas -/
def for_all_qe_qf : @qe_qf L A Γ _ := by { intros φ, existsi φ, refl }

/- If a theory has quantifier elimination on φ₁ φ₂ then it has quantifier 
   elimination on (φ₁ or φ₂) -/
lemma equiv_qf_or_equiv_qf : equiv_qf A Γ φ₁ → equiv_qf A Γ φ₂ → equiv_qf A Γ (φ₁ or φ₂) := begin
   intros h_φ₁ h_φ₂,
   rcases h_φ₁ with ⟨φ₁', h₁⟩,
   rcases h_φ₂ with ⟨φ₂', h₂⟩,
   apply Eq_equiv_qf A (R_Eq_Or_ ⟨h₁.mpr, h₁.mp⟩ ⟨h₂.mpr, h₂.mp⟩),
   existsi (qf.o φ₁' φ₂'), refl,
end

lemma qe_ecl1_qe_edcl1 : (@qe_ecl1 L A Γ _) → (@qe_edcl1 L A Γ _) := begin
   intros h₁ φ,
   cases φ,
   {  existsi (φ : qf L), refl, },
   {  induction φ_ᾰ_1,
      rcases (h₁ (ecl1.ex φ_ᾰ φ_ᾰ_1)) with ⟨φ₂, h₂⟩,
      apply Eq_equiv_qf A ⟨h₂.mpr, h₂.mp⟩,
      existsi φ₂, refl,
      rcases φ_ᾰ_1_ih_ᾰ with ⟨φ₂, h₂⟩,
      rcases φ_ᾰ_1_ih_ᾰ_1 with ⟨φ₃, h₃⟩,
      apply Eq_equiv_qf A ⟨R_ ExOrOut, R_ ExOrIn⟩,
      apply Eq_equiv_qf A (R_Eq_Or_ ⟨h₂.mpr, h₂.mp⟩ ⟨h₃.mpr, h₃.mp⟩),
      existsi qf.o φ₂ φ₃, refl,
   },
end

lemma qe_edcl1_qe_qdcl1 : (@qe_edcl1 L A Γ _) → (@qe_qdcl1 L A Γ _) := begin
   intros h₁ φ,
   induction φ,
   {  existsi (φ : qf L), refl, },
   { 
      rcases (qf_equiv_dcl A Γ (qf.n φ_ᾰ_1)) with ⟨φ₂, h₂⟩,
...
lemma qe_qdcl1_qe_dnf : (qe_qdcl1 A Γ) → (qe_dnf A Γ) := begin
   intros h₁ φ₁,
   induction φ₁,
   {  existsi (φ₁ : qf L), refl, },
   {  cases φ₁_ᾰ_1,
...
lemma qe_ecl1_qe : (qe_ecl1 A Γ) → (qe A Γ) := begin
   intro h,
   have h_dnf : qe_dnf A Γ := 
      by { apply qe_qdcl1_qe_dnf, apply qe_edcl1_qe_qdcl1, apply qe_ecl1_qe_edcl1, assumption },
   intro φ₁,
   rcases (for_all_equiv_dnf A Γ φ₁) with ⟨φ₂, h₂⟩,
   apply Eq_equiv_qf A ⟨h₂.mpr, h₂.mp⟩,
...
def decidable_sent (φ : sentence L) : Prop := ((A∣Γ ⊢ φ) ↔ (A∣Γ ⊢ F)) ∨ ((A∣Γ ⊢ φ) ↔ (A∣Γ ⊢ T))
